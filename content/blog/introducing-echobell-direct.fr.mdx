...
[Echobell Direct](/fr/docs/direct) est un mode d'alerte personnel qui court-circuite entièrement la configuration des canaux. Vous créez une clé Direct, obtenez une URL Webhook, et envoyez un `title` + `body` en POST pour recevoir une notification instantanée sur votre appareil. Pas d'abonnés, pas de modèles, pas de configuration de routage.
...
Si vous avez besoin de notifications partagées, de modèles réutilisables ou d'une livraison conditionnelle, utilisez plutôt les [Canaux via Webhooks](/fr/docs/webhook).
...
Référence complète des champs : [API Direct](/fr/docs/direct)
...
Voir [Types de notifications](/fr/docs/notification) pour les détails complets.
...
Pour les agents IA et les tâches asynchrones longues, Direct est un moyen simple d'être notifié quand le job se termine plutôt que de surveiller manuellement. Voir aussi : [WebhookMCP + Echobell](/fr/blog/get-notified-with-webhook-mcp).
...
Utilisez les Canaux quand les alertes doivent atteindre plusieurs personnes ou quand vous souhaitez un formatage cohérent à travers un service. [Documentation Modèles](/fr/docs/template) · [Qu'est-ce qu'Echobell](/fr/docs/what-is-echobell)
...
Commencez avec le [guide Direct](/fr/docs/direct). Quand votre workflow nécessite collaboration ou modèles, les [Canaux](/fr/docs/webhook) constituent une étape suivante naturelle.

## Articles connexes

- [Documentation de l'API Direct](/fr/docs/direct) — référence complète des champs et exemples
- [WebhookMCP + Echobell](/fr/blog/get-notified-with-webhook-mcp) — notifications pour les tâches IA avec Direct
- [Comment contourner iOS Focus Mode pour les alertes critiques](/fr/blog/how-to-bypass-ios-focus-mode-for-critical-alerts) — choisir le bon niveau d'urgence pour les notifications
- [Notifications webhook Zapier](/fr/blog/zapier-webhook-notifications-to-phone) — workflows d'automatisation pour les non-développeurs
