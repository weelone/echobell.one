...
n8nはワークフロー自動化が得意ですが、組み込みの通知機能は弱いです。[Echobell](https://apps.apple.com/app/apple-store/id6743597198?pt=128151925&ct=blog-n8n-echobell-automation-hub-ja&mt=8) / [Google Play](https://play.google.com/store/apps/details?id=one.echobell.echobellandroid)と組み合わせることでその弱点を補えます。n8nがロジックを処理し、Echobellが実際に届くプッシュアラートや電話着信を配信します。
...
n8n経由でHome Assistantの重要イベント（水漏れセンサー、煙感知器、外出中のドア・窓センサー）をEchobellに転送します。詳細は[Home Assistantインテグレーションガイド](/ja/blog/home-assistant-notifications-with-echobell)を参照してください。
...
Echobellの[条件](/ja/docs/conditions)機能を使って配信レイヤーでフィルタリング:
...
まず1つのワークフローから始め、徐々に拡張していきましょう。[Webhookドキュメント](/ja/docs/webhook)にはEchobell APIの全リファレンスが記載されています。
