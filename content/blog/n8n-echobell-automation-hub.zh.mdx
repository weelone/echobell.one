...
n8n 的工作流自动化能力很强，但内置通知功能较弱。搭配 [Echobell](https://apps.apple.com/app/apple-store/id6743597198?pt=128151925&ct=blog-n8n-echobell-automation-hub-zh&mt=8) / [Google Play](https://play.google.com/store/apps/details?id=one.echobell.echobellandroid) 可以填补这一空缺：n8n 负责处理逻辑，Echobell 负责发送真正能触达你的推送通知或来电。
...
通过 n8n 将 Home Assistant 的关键事件（漏水传感器、烟雾探测器、离家时的门窗传感器）转发给 Echobell。详见 [Home Assistant 集成指南](/zh/blog/home-assistant-notifications-with-echobell)。
...
Echobell 的[条件](/zh/docs/conditions)功能可在投递层进行过滤：
...
从一个工作流开始，逐步扩展。[Webhook 文档](/zh/docs/webhook)涵盖完整的 Echobell API。
