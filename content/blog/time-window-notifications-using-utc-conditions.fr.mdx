...
Prérequis : [Conditions](/fr/docs/conditions), [Modèles](/fr/docs/template), [Webhook](/fr/docs/webhook), [Déclencheurs email](/fr/docs/email-trigger).
...
Utilisez-les directement dans les [Conditions](/fr/docs/conditions) (sans `{{ }}`). Utilisez-les avec `{{ }}` dans les [Modèles](/fr/docs/template).
...
Les Conditions sont des expressions simples — ne les entourez pas de `{{ }}`. Voir [Conditions → Opérateurs supportés](/fr/docs/conditions#supported-operators).
...
Combinez avec les données provenant des [Webhooks](/fr/docs/webhook) ou des [Déclencheurs email](/fr/docs/email-trigger) :
...
- Ajoutez une condition de fenêtre temporelle à un canal existant : [Conditions](/fr/docs/conditions)
- Personnalisez le contenu des notifications avec des variables : [Modèles](/fr/docs/template)
