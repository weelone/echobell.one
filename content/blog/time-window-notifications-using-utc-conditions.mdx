...
New to conditions or templates? See the guides: [Conditions](/en/docs/conditions) and [Templates](/en/docs/template). Trigger sources: [Webhook](/en/docs/webhook) and [Email Triggers](/en/docs/email-trigger).
...
You can reference them directly in [Conditions](/en/docs/conditions) (no `{{ }}`) and in [Templates](/en/docs/template) (with `{{ }}`).
...
Remember: conditions are plain expressions—don’t wrap them in `{{ }}`. See all operators in [Conditions → Supported Operators](/en/docs/conditions#supported-operators).
...
You can also mix time with data you send via [Webhook](/en/docs/webhook) or [Email Triggers](/en/docs/email-trigger). For example:
...
- Polish your notification content with variables: [Templates](/en/docs/template)
