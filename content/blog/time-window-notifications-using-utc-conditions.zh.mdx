...
前提条件：[条件](/zh/docs/conditions)、[模板](/zh/docs/template)、[Webhook](/zh/docs/webhook)、[邮件触发](/zh/docs/email-trigger)。
...
在[条件](/zh/docs/conditions)中直接使用（无需 `{{ }}`）。在[模板](/zh/docs/template)中用 `{{ }}` 包裹使用。
...
条件是普通表达式——不要用 `{{ }}` 包裹。运算符说明参见[条件 → 支持的运算符](/zh/docs/conditions#supported-operators)。
...
与来自 [Webhook](/zh/docs/webhook) 或[邮件触发](/zh/docs/email-trigger)的数据混合使用：
...
- 为现有频道添加时间窗口条件：[条件](/zh/docs/conditions)
- 用变量定制通知内容：[模板](/zh/docs/template)
- 将工作时间频道与值班频道分开
- 值班频道在非工作时段设置为来电通知类型
