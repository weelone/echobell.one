...
- Notifications and priorities (Normal, Time Sensitive, Calling) → [/docs/notification](/en/docs/notification)
...
2. Choose the notification type. For urgent alerts, select **Calling**. For less critical alerts, you can use **Time Sensitive** or **Normal**. See [/docs/notification](/en/docs/notification).
...
3. Customize the content under Templates (optional but recommended). Your call can literally say what happened. Pair it with TradingView variables you’ll send via webhook:
   ...
   More on variables and expressions: [/docs/template](/en/docs/template)

4. Copy your channel’s Webhook URL from the channel details. This is the address TradingView will call. Treat it like a secret. Details: [/docs/webhook](/en/docs/webhook)
...
- `externalLink` is optional: when included, Echobell shows a clickable link in the local record. See [/docs/notification](/en/docs/notification).
...
Learn all template options here → [/docs/template](/en/docs/template)
...
- CI/CD and repo health → [/blog/github-actions-notifications](/en/blog/github-actions-notifications)
- Infrastructure and monitoring → [/blog/grafana-call-notification](/en/blog/grafana-call-notification)

You can also explore feature pages:

- Webhooks → [/features/webhooks](/en/features/webhooks)
- Call notifications → [/features/call-notifications](/en/features/call-notifications)
